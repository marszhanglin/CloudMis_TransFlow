...
func uploadflow(c echo.Context) error {
	...
	//获取用户信息   为了校验店铺及运营商
	isUserExit, user := dbQueryUserByName(userName)
	if !isUserExit {
	...
	flows.ShopId = getFormParam(c, "shopId")
	...
	// 3 判断运营商与店铺是否匹配
	if user.ShopId != flow.ShopId {
		errResponse := getResPonse("4615", "店长与门店编号不符")
		return c.JSON(http.StatusOK, errResponse)
	}
...
func uploadflowbatch(c echo.Context) error {
	...
	succResponse := getResPonse("00", "succ1")
	return c.JSON(http.StatusOK, succResponse)
...
func vo2poTransFlow(flow *TransFlowVO) (isSucc bool, rtnflow *TransFlow, errMsg string) {
	...
	var shopErr, uploadTimeErr, transTimeErr, amountErr, transAmountErr error
	poTransFlow.ShopId, shopErr = strconv.ParseInt(flow.ShopId, 10, 64)
	poTransFlow.FlowNo = flow.FlowNo
	poTransFlow.DeviceSn = flow.DeviceSn
	poTransFlow.UploadTime, uploadTimeErr = time.Parse("20060102150405", flow.UploadTime)
	poTransFlow.TransTime, transTimeErr = time.Parse("20060102150405", flow.TransTime)
...
type TransFlowVO struct {
	...
	ShopId         string `json:"shopId" validate:"required,gte=1"`
	...
	MerchantId     string `json:"merchantId" validate:"required,max=64"`
